...
const TodoListModal = ({ showModal, handleCloseModal, addTask, housemates, isEdit, editTask, editAssignedTo }) => {
  ...
  return (
    ...
    >
      <form onSubmit={submitTask} className="modal-form bg-white dark:bg-neutral-900 rounded-lg shadow-2xl ">
      <header className=" bg-gray-100 dark:bg-neutral-800 px-4 py-2 flex justify-between items-center">
        ...
        >
          ...
          {housemates.map((housemate, index) => (
            <option key={index} value={housemate}>
              {housemate}
            </option>
          ))}
        </select>
        <footer className='flex flex-row justify-between items-center border-t p-3 pl-5 mt-5'>
            ...
            <button type="submit" className="bg-blue-500 hover:bg-blue-600 px-6 py-2 rounded text-white">
              {isEdit ? 'Save Changes' : 'Add Task'} {/* Update button text based on add/edit mode */}
            </button>
        </footer>
      </form>
    </Modal>
